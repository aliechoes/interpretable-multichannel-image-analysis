...
logging.basicConfig(filename=os.path.join(opt.log_dir, 'interpretation_metrics_output_{}.txt'.format(timestamp)), level=logging.DEBUG)
...
if __name__ == '__main__':
    ...
    for channel, y_pred_per_ch in y_pred_per_channel.items():
        ...
        for y_pred_per_shuffle in y_pred_per_im:
            f1_scores_per_shuffle.append(f1_score(y_true, y_pred_per_shuffle, average=None, labels=np.arange(opt.num_class)))
        ...
        fig.savefig(os.path.join(opt.path_to_save_results,
                                 "bp-shuffle_method-model-{}-channel-{}.png".format(str(os.path.basename(os.path.normpath(opt.path_to_model))), str(channel))))
        #breakpoint()
